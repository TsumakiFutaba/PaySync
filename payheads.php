...
<body class="hold-transition skin-blue sidebar-mini">
	<div class="wrapper">
		...
		<div class="modal fade in" id="PayheadsModal">
					...
					<form method="POST" role="form" data-toggle="validator" id="payhead-form">
                            <div class="modal-body">
                                <div class="form-group">
                                    ...
                                    <small id="nameHelp" class="form-text text-danger" style="display: none;">Please enter only alphabets and spaces.</small>
                                </div>
                                <div class="form-group">
                                    ...
                                    <small id="descHelp" class="form-text text-danger" style="display: none;">Please enter only alphabets and spaces.</small>
                                </div>
                                <div class="form-group">
                            </div>
	...
	<script>
    document.getElementById('payhead_name').addEventListener('input', function () {
        var validInput = /^[A-Za-z\s]+$/.test(this.value);
        var errorField = document.getElementById('nameHelp');
        if (!validInput) {
            errorField.style.display = 'block';
            this.setCustomValidity('Please enter only alphabets and spaces.');
        } else {
...
<script>
    document.getElementById('payhead_desc').addEventListener('input', function () {
        var inputValue = this.value;
        var validInput = /^[A-Za-z0-9\s]*$/.test(inputValue);
        var errorField = document.getElementById('descHelp');
        if (!validInput) {
            errorField.style.display = 'block';
            this.setCustomValidity('Please enter only alphabets, numbers, and spaces.');
        } else {
            errorField.style.display = 'none';
            this.setCustomValidity('');
        }
        this.value = inputValue.replace(/[^\w\s]/gi, '');
    });
</script>
